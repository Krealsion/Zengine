...
of `layout.new`"`, case `"WUX-11/SC-8: at the minimum width the `+` yields to the tab and the
status"`.
WHY — `agents/decisions/the-layouts-pane.md`
...
cell, the name, one cell"`, case `"QR-15/SC-4: switching the live layout moves nothing to the
right of it"`, case `"QR-15/SC-7: the closing marker belongs to the layout it closes"`.
WHY — `agents/decisions/the-layouts-pane.md`
