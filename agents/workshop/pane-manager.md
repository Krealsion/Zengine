...
`workshop/screen_pane_state.cpp` `picker_entry_text`; `tests/test_workshop_panels_creator.cpp`
case `"WUX-14/SC-2: the WUX-13 surface is the Pane Manager, and its durable key did not move"`;
`tests/test_workshop_panels.cpp` case `"WUX-13/SC-2: the Pane Editor is a built-in, and its list
is the picker's population"`; `tests/test_workshop_screen.cpp` case `"INTR-0 bounded extension: a
provider's long name is MARKED in the picker, not cut"`.
...
PROVEN BY — `workshop/screen.hpp` `PaneEditor`, `Session::pane_editor`;
`workshop/weave_pane_editor.cpp` `choose_subject`, `pane_editor_press`;
`tests/test_workshop_panels.cpp` case `"WUX-13/SC-1: the subject is chosen, and interacting inside
the editor does not retarget it"`, case `"WUX-13/SC-15: the Pane Editor can be its own subject,
and its own rows do not retarget it"`, case `"WUX-13/SC-13: a Pane Editor edit survives a restart
through the session, and the subject does not"`.
WHY — `agents/decisions/a-subject-is-not-a-selection.md`
...
PROVEN BY — `workshop/weave_pane_editor.cpp` `repair_pane_editor_subject`;
`workshop/weave_arrange.cpp` `forget_removed_selection`; `workshop/setup.hpp` `inventory_rows`;
`workshop/screen.hpp` `PaneEditor::subject`; `tests/test_workshop_panels.cpp` case `"WUX-13: the
subject stands across a layout switch, and clears only when nothing names it"`, case
`"WUX-13/SC-9: a closed pane and an unresolved row are subjects with honest facts"`.
WHY — `agents/decisions/a-subject-is-not-a-selection.md`
...
PROVEN BY — `workshop/screen_pane_editor.cpp` `write_pane_axis`, `pane_window_base`;
`workshop/setup.hpp` `author_pane_window`, `reset_pane_place`; `workshop/weave_session.cpp`
`apply_setup`; `workshop/weave_arrange.cpp` `spend_pane_action`, `managed_window_base`;
`workshop/weave_panels.cpp` `toggle_participation`, `choose_panel`; `workshop/weave_seam.cpp`
`editing_key`; `tests/test_workshop_panels.cpp` case `"WUX-13/SC-6+SC-11: a typed place moves
Layouts through the gesture door, and its tabs follow"`, case `"WUX-13/SC-6: a typed place reseats
the stack through `apply_setup`"`, case `"WUX-13/SC-12: moving, resizing and closing Layouts
through the editor leaves the reservation alone"`.
WHY — `agents/decisions/a-subject-is-not-a-selection.md`
...
`workshop/screen.hpp` `subs_of_device_amount`, `FaceAmount`; `workshop/screen_pane_editor.cpp`
...
LAW — `KeyContext::kDraft` is the manager's draft context, and the draft under the keys is resolved by one chain, so a change of selection cannot touch it; the caret it keeps is its own body's.
...
- `pane_editor_draft_live` is the one question; the object inspector's `draft_live` retired.

PROVEN BY — `workshop/screen_arrange.cpp` `keyboard_context_beneath_menu`,
`pane_editor_has_keyboard`, `pane_editor_draft_live`; `workshop/weave_run.cpp`
`finish_draft_first`; `workshop/weave_seam.cpp` `editing_row`,
...
PROVEN BY — `workshop/weave_pane_editor.cpp` `pane_editor_move_in`, `pane_editor_wheel`;
