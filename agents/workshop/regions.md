...
PROVEN BY — `workshop/screen.hpp` `detail::fit`, `paint`; `tests/test_workshop_screen.cpp` case
`"QR-3: the name's bound is the OBJECT'S resolved width, clipped by the workspace"`, case `"QR-3:
no part of a name is drawn outside the material it names"`, case `"TYPE-1: a tiny object shows
its name in CELLS, and no rule was written to say so"`.
WHY — `agents/decisions/semantic-text-owns-its-room.md`
