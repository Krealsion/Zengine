...
- **`zen.ListLoaded` is not the load capability.** A Loom grant is per
  ...
  (an undeclared shape is still sendable under a grant) and `Kernel::load` binds `allow_any()`
  to every library it opens — so a declaration proves nothing on its own and is not quoted as
  though it did.
