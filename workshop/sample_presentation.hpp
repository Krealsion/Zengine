...
/// A double, written so it reads back as itself: fifteen significant digits, seventeen only if
/// fifteen did not round-trip.
...
/// One cell, as the tail of a line that already carries its label, or as a block beneath it for
/// the two kinds with interiors; returns the inline tail. It switches on the cell's own kind,
/// never the field's declared kind, which is what makes it total: the declaration is a claim and
/// the storage is the fact.
...
/// Every field of one message, in the schema's order, with names padded to the widest at this
/// level only: a nested message is a different column.
...
/// The whole of what a sample looks like, as logical lines; the first is the schema's identity,
/// which the rest are about. Pure and total: a value renders the same anywhere, and every value
/// the public machinery admits yields lines, never an exception.
