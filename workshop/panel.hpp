...
// which office to ask. A later panel that presents something with no weave
// behind it at all — the document, the object list — fits this catalog without
// changing it, which is the test of whether the split was real.
//
...
//     Builder panel is open two keys that were previously unbound mean
//     something. That is one conditional in `command()`, and it is the same
//     answer the terminal overlay gave to the same question.
//   - no multi-instance policy. Opening a kind that is already open is refused
//     with a sentence, because one live Builder is what BLD-0 needs and a policy
...
/// The KINDS of panel this Workshop can present. One so far.
///
