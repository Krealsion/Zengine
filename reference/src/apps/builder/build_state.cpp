...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(Text::Config, font_name, font_size);
...
//  _playground->set_position(PositionTo::RELATIVE, 0, PositionTo::RELATIVE, 0);
  ...
  _details_pane = new ScrollView();
  _details_pane->set_name("details_pane");
  _details_pane->set_vertical();
  _details_pane->set_position_y(PositionTo::BOTTOM);
  ...
  const auto left_pane = new CustomLayout();
  left_pane->set_name("left_pane");
  left_pane->set_vertical();
  left_pane->set_size(SizeTo::PARENT_PERCENT, 0.2f, SizeTo::PARENT);
  left_pane->add_child(_create_pane);
  left_pane->add_child(_tree_pane);
  left_pane->add_child(_details_pane);

  _build_root = new CustomLayout();
  _build_root->set_as_root();
  _build_root->set_name("parent/root");
  _build_root->set_background_color(Color(210, 210, 210));
  _build_root->set_size(SizeTo::PARENT_PERCENT, 0.7f, SizeTo::PARENT);

  _root_layout->set_horizontal();
  _root_layout->add_child(left_pane);
  _root_layout->add_child(_build_root);
...
BuildState::~BuildState() {
  if (_selection && !_selection->_parent) {
    delete _selection;
  }
  if (_highlight && !_highlight->_parent) {
    delete _highlight;
  }
}
...
void BuildState::update_build_root() const {
...
void BuildState::update() {
  ...
  if (update_timer.is_time()) {
    ...
    if (Input::is_mouse_button_down(MIDDLE)) {
      auto log_element = [](CustomLayout* element) {
        Logger::log(LogLevel::INFO, "Element: " + element->get_name() +
                                    " at (" + std::format("{:g}",element->get_position().get_x()) + ", " + std::format("{:g}",element->get_position().get_y()) + ")" +
                                    " with size (" + std::format("{:g}",element->get_width()) + ", " + std::format("{:g}",element->get_height()) + ")");
      };
      log_element(_build_root);
      for (const auto& child : _build_root->_children) {
        log_element(child);
      }
      _build_root->reset();
    }
    ...
    if (Input::is_mouse_button_down(LEFT)) {
      ...
      // auto* component_from_root = get_component(mouse_pos, _build_root, true);
      // if (component_from_root != _build_root) {
      //   component_from_root->click();
      // } else {
      //   if (auto* component = get_component(mouse_pos, _build_root); component != _selection) {
...
void BuildState::delete_selection() {
  if (_selection && _selection != _build_root) {
...
CustomLayout* BuildState::get_component(const Vector2 mouse_pos, CustomLayout* layout, const bool only_callback_registered) {
  for (const auto& child : layout->_children) {
    if (child->is_visible() && child->get_owned_destination().contains(mouse_pos)) {
      if (auto* found = get_component(mouse_pos, child, only_callback_registered); !only_callback_registered || (found->has_callback())) {
        return found;
      }
    }
  }
...
void BuildState::update_highlight() {
  ...
  if (hovered != _selection && hovered != default_root) {
    if (!_highlight) {
      _highlight = new CustomLayout(); // TODO ensure that this is properly managed and deleted
      _highlight->set_name("highlight");
      _highlight->set_as_root();
      _highlight->set_background_color(Color(0, 255, 255, 58)); // TODO Highlight Color
    }
...
void BuildState::refresh_tree() {
  ...
  _tree_pane->add_child(create_component_tree(_build_root));
}

CustomLayout* BuildState::create_create_pane() {
  auto* layout = new CustomLayout();
  layout->set_name("create_pane");
  layout->set_size(SizeTo::PARENT, SizeTo::CHILDREN_PERCENT, 1.1);
  layout->set_vertical();
  ...
  auto add_button = [&](const std::string& text, auto create_func) {
    ...
    button->set_on_click_callback(Action<>([this, create_func]() {
      if (_selection == nullptr) {
        enter_selection_mode(_build_root);
      }
      if (_selection) {
  ...
  add_button("Create Layout", []() {
    auto* layout = new CustomLayout();
    layout->set_size(SizeTo::STATIC, 50, SizeTo::STATIC, 50);
    layout->set_background_color(Color(SDL_rand(255), SDL_rand(255), SDL_rand(255)));
    return layout;
  });
  add_button("Create Button", []() {
    auto* button = new Button();
    button->set_text("Button Text");
    return button;
  });
  add_button("Create Text", []() {
    auto* text = new Text();
    text->set_text("Text");
    return text;
  });
  add_button("Create TextBox", []() {
    auto* textbox = new TextBox();
    textbox->set_text("Textbox Text");
    return textbox;
  });
  add_button("Create ScrollView", []() {
  ...
  layout->add_child(delete_button);
  ...
  save_button->set_size(SizeTo::FILL, SizeTo::CHILDREN);
  ...
  load_button->set_size(SizeTo::FILL, SizeTo::CHILDREN);
  ...
  save_load_container->set_size(SizeTo::FILL, SizeTo::CHILDREN);
  ...
  layout->add_child(save_load_container);
...
CustomLayout* BuildState::create_details_row(const std::string& label, const TextBox::TextBoxFilter filter, const std::string& value, Action<const std::string&>&& callback) {
  auto* layout = new CustomLayout();
...
CustomLayout* BuildState::create_dropdown_row(const std::string& label, const std::vector<std::string>& options, const int selected_index, Action<int>&& callback) {
  auto* layout = new CustomLayout();
...
CustomLayout* BuildState::create_basic_details(CustomLayout* subject) {
  ...
  layout->add_child(create_details_row("Width Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_size_to_width_value), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Height Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_size_to_height_value), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_dropdown_row("Position X", {"ABSOLUTE", "RELATIVE", "LEFT", "RIGHT", "CENTER", "PARENT_CONTROLLED"}, // TODO VERY IMPORATNT generate this automatically, so we can add values as desired
  ...
  layout->add_child(create_dropdown_row("Position Y", {"ABSOLUTE", "RELATIVE", "TOP", "BOTTOM", "CENTER", "PARENT_CONTROLLED"},
  ...
  layout->add_child(create_details_row("Padding Top Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_padding_value_top), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Padding Bottom Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_padding_value_bottom), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Padding Right Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_padding_value_right), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Padding Left Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_padding_value_left), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Margin Top Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_margin_value_top), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Margin Bottom Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_margin_value_bottom), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Margin Right Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_margin_value_right), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
  ...
  layout->add_child(create_details_row("Margin Left Value", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER),
                                       std::format("{:g}",subject->_margin_value_left), Action<const std::string&>([this, subject](const std::string& value) {
        if (!value.empty()) {
...
CustomLayout* BuildState::create_text_details(Text* text) {
  ...
  layout->add_child(create_details_row("Font Size", TextBox::TextBoxFilter(TextBoxFilterType::DATA_TYPE, DataType::NUMBER), std::format("{:g}",text->get_font_size()),
                                       Action<const std::string&>([this, text](const std::string& value) {
                                         if (!value.empty()) {
...
void BuildState::save_to_json(const std::string& file_path) {
  json j = serialize_component(_build_root);
...
void BuildState::load_from_json(const std::string& file_path) {
  ...
  // TODO do not ovewrite build root, instead read in components and add them to build root
  _selection = _build_root;
  refresh_details();
  refresh_tree();
...
nlohmann::json BuildState::serialize_component(CustomLayout* component) {
  ...
  j["position_to_x_value"] = static_cast<double>(component->_position_to_x_value);
  j["position_to_y"] = component->_position_to_y;
  j["position_to_y_value"] = static_cast<double>(component->_position_to_y_value);
  j["layout"] = component->_layout;
  j["child_spacing"] = static_cast<double>(component->_child_spacing);
...
CustomLayout* BuildState::deserialize_component(const json& j) {
  std::string type = j["type"];
  CustomLayout* component;

  if (type.find("Button") != std::string::npos) {
    component = new Button();
    dynamic_cast<Button*>(component)->set_text(j["text"]);
    ...
    auto* textbox = dynamic_cast<TextBox*>(component);
