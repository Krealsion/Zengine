...
public:
  ...
  void update_build_root() const;
  void draw(GameGraphics& g) override;
  static CustomLayout* get_component(Vector2 mouse_pos, CustomLayout* layout, bool only_callback_registered = false);
...
private:
  ...
  CustomLayout* deserialize_component(const nlohmann::json& j);
  ...
  CustomLayout* _dragged_component = nullptr;
  Vector2 _drag_start_pos;

  // UI panels
  ScrollView* _create_pane;
  ScrollView* _tree_pane;
  ScrollView* _details_pane;
};
