...
void CustomLayout::add_child(CustomLayout* child, const int position) {
  if (child == nullptr) {
...
void CustomLayout::remove_child(CustomLayout* child) {
  auto it = std::ranges::find_if(_children, [child](const CustomLayout* c) { return c == child; });
  if (it != _children.end()) {
...
void CustomLayout::set_width(const SizeTo size_to, const double value) {
  _size_to_width = size_to;
  _size_to_width_value = value;
  _width_current = false;
  for (const auto& child : _children) {
    if (
      child->_size_to_width == SizeTo::FILL ||
      child->_size_to_width == SizeTo::PARENT_PERCENT ||
      child->_size_to_width == SizeTo::PARENT) {
      child->_width_current = false;
    }
  }
}

void CustomLayout::set_height(const SizeTo size_to, const double value) {
  _size_to_height = size_to;
  _size_to_height_value = value;
  _height_current = false;
  for (const auto& child : _children) {
    if (
      child->_size_to_height == SizeTo::FILL ||
      child->_size_to_height == SizeTo::PARENT_PERCENT ||
      child->_size_to_height == SizeTo::PARENT) {
      child->_height_current = false;
      }
  }
}

void CustomLayout::set_size(const SizeTo size_to_width, const SizeTo size_to_height) {
  set_size(size_to_width, 0, size_to_height, 0);
}

void CustomLayout::set_size(SizeTo size_to_width, const double width_value, const SizeTo size_to_height, const double height_value) {
  set_width(size_to_width, width_value);
  set_height(size_to_height, height_value);
}

void CustomLayout::set_size(const SizeTo size_to_width, const SizeTo size_to_height, const double height_value) {
  set_size(size_to_width, 0, size_to_height, height_value);
}

void CustomLayout::set_size(const SizeTo size_to_width, const double width_value, const SizeTo size_to_height) {
...
void CustomLayout::click_location(const Vector2& location) {
  if (!get_owned_destination().contains(location)) {
    return;
  }
  if (const auto found = find_component(location, true); found != this) {
    found->click();
  }
}

CustomLayout* CustomLayout::find_component(const Vector2& mouse_pos, const bool only_callback_registered) {
  if (!get_owned_destination().contains(mouse_pos)) {
    return nullptr;
  }
  for (const auto& child : _children) {
    if (child->is_visible() && child->is_enabled() && child->get_owned_destination().contains(mouse_pos)) {
      if (auto* found = child->find_component(mouse_pos, only_callback_registered); !only_callback_registered || (found->has_callback())) {
        return found;
      }
    }
  }
...
double CustomLayout::get_width() {
    ...
    case SizeTo::CHILDREN_PERCENT:
      if (_children.empty()) {
        ...
        for (const auto& child : _children) {
          ...
          const double child_width = child->get_width();
      ...
      break;
    case SizeTo::FILL: {
      if (_parent->_layout == Layout::HORIZONTAL) {
        int fill_count = 0;
        double non_fill_children_width = 0;
        for (const auto& child: _parent->_children) {
          if (non_fill_children_width == 0) {
            non_fill_children_width += _parent->_child_spacing;
          }
        ...
        _width = (_parent->get_inside_width() - non_fill_children_width) / fill_count;
      } else {
        _width = _parent->get_inside_width();
      }
      break;
    }
    case SizeTo::STATIC:
  ...
  if (const auto minimum_with_mp = _min_width + get_margin_left() + get_margin_right() + get_padding_left() + get_padding_right(); _width < minimum_with_mp) {
    _width = minimum_with_mp;
  }
...
double CustomLayout::get_height() {
  ...
  switch (_size_to_height) {
    ...
    case SizeTo::FILL: {
      int fill_count = 0;
      double non_fill_children_height = 0;
      for (const auto& child: _parent->_children) {
        if (non_fill_children_height == 0) {
          non_fill_children_height += _parent->_child_spacing;
        }
      ...
      _height = (_parent->get_inside_height() - non_fill_children_height) / fill_count;
      break;
    }
    case SizeTo::STATIC:
      _height = _size_to_height_value;
  ...
  if (const auto minimum_with_mp = _min_height + get_margin_left() + get_margin_right() + get_padding_left() + get_padding_right(); _height < minimum_with_mp) {
    _height = minimum_with_mp;
  }
...
void CustomLayout::set_position(const PositionTo position_to_x, const PositionTo position_to_y) {
  set_position_x(position_to_x, 0);
  set_position_y(position_to_y, 0);
}

auto CustomLayout::set_position(const PositionTo position_to_x, const int value_x, const PositionTo position_to_y, const int value_y) -> void {
  set_position_x(position_to_x, value_x);
  set_position_y(position_to_y, value_y);
}

void CustomLayout::set_position_x(const PositionTo position_to_x) {
  set_position_x(position_to_x, 0);
}
void CustomLayout::set_position_x(const PositionTo position_to_x, const int value_x) {
...
void CustomLayout::set_position_y(const PositionTo position_to_y) {
  set_position_y(position_to_y, 0);
}

void CustomLayout::set_position_y(const PositionTo position_to_y, const int value_y) {
...
Rectangle CustomLayout::get_owned_destination() {
  const auto owned = Rectangle(get_position().get_x(), get_position().get_y(), get_width(), get_height());
  return owned;
}
...
void CustomLayout::_on_size_changed() {
  ...
  _height_current = false;
  for (const auto& child : _children) {
    child->_on_size_changed();
  }
  if (_parent) {
    _parent->_child_size_changed();
  }
}
