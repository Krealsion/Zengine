...
public:
  ...
  Text();
  explicit Text(const Config& config); // TODO use this at some point

  void set_wrap(bool should_wrap);

  void set_text(const std::string& text) const;
  void set_font(const std::string& font) const;
  void set_font_size(float font_size) const;
  void set_font_color(Color font_color) const;
