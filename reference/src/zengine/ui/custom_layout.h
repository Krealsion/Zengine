...
public:
  ...
  CustomLayout(CustomLayout&&) noexcept = default;
  CustomLayout& operator=(CustomLayout&&) noexcept = default;
  ...
  void set_child_spacing(const int spacing) { _child_spacing = spacing; }
  ...
  virtual void set_position_x(PositionTo position_to_x);
  virtual void set_position_y(PositionTo position_to_y);
  ...
  virtual void set_background_color(const Color background_color) { _has_background_color = true; _background_color = background_color; }
  Color get_background_color() const { return _background_color; }

  CustomLayout* find_component(const Vector2& mouse_pos, const bool only_callback_registered);
  ...
  void click_location(const Vector2& location);
  virtual void click();
...
protected:
  ...
  // TODO consider adding a SizeTo enum for these to allow for percentage based constraints possibly match general sizing options
