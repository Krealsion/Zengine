...
Button::Button() {
  ...
  _inner->set_margin(2, 2, 2, 2); // TODO configuration
  _inner->add_child(_text);

  this->CustomLayout::set_background_color(_border_color);
  // this->CustomLayout::set_padding(2, 2, 2, 2);// TODO configuration
  this->CustomLayout::add_child(_inner);
}
...
void Button::set_text_position(const PositionTo position_to_x, const PositionTo position_to_y) const {
  set_text_position(position_to_x, 0, position_to_y, 0);
}

void Button::set_text_position(const PositionTo position_to_x, const int value, const PositionTo position_to_y, const int value_y) const {
  _text->set_position(position_to_x, value, position_to_y, value_y);
}
...
void Button::draw(GameGraphics& game_graphics) {
  if (_mouse_hovered) {
    const auto color = _default_background_color;
    if (_auto_hover_color) {
      const auto auto_color = Color(color.get_red() + (255 - color.get_red()) / 2,
...
void Button::set_border_color(const Color color) {
  _border_color = color;
  this->CustomLayout::set_background_color(_border_color);
}
void Button::set_border(const int border_size) const {
  _inner->set_margin(border_size, border_size, border_size, border_size);
}
