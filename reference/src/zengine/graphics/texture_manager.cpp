...
std::shared_ptr<Texture> TextureManager::get_text_texture(const std::string& text, const std::string& font_path, const float text_size, const Color text_color) {
  const std::string usable_font_path = "Resources//TTFs//" + font_path;
  TTF_Font* font = TTF_OpenFont(usable_font_path.c_str(), text_size);
  const SDL_Color sdl_color = {.r = text_color.get_red(), .g = text_color.get_green(), .b = text_color.get_blue(), .a = text_color.get_alpha()};
...
Vector2 TextureManager::get_text_size(const std::string& text, const std::string& font_path, const float text_size) {
  return get_text_size(text, font_path, text_size, 0);
}

Vector2 TextureManager::get_text_size(const std::string& text, const std::string& font_path, const float font_size, const int max_width) {
  const std::string usable_font_path = Utility::getResourcePath("TTFs/" + font_path).string();
  TTF_Font* font = TTF_OpenFont(usable_font_path.c_str(), font_size);
  if (!font) {
    // TODO Add error handling
    const std::string error_message = "Failed to load font: " + usable_font_path + " Error: " + SDL_GetError();
