...
| [workshop/external-host.md](workshop/external-host.md) | **walkthrough**: drive Workshop from another Loom host — the guests file that admits one, a `links` row in `loom-host`, the probe weave that opens an input session, presses keys, takes a picture and keeps the exchange in its own history, and the Connections pane that shows who is connected |
